class SegTree(T)
  ...
  def initialize(s : Int32, @op : Proc(T, T, T))
    @size = 1
    while @size < s
      @size *= 2
    end
    @ar = Array.new(@size * 2, T.zero)
  end

  def initialize(init : Array(T), @op : Proc(T, T, T))
    @size = 1
    while @size < init.size
      @size *= 2
    end
    @ar = Array.new(@size, T.zero)
    @ar.concat(init)
    @ar.concat([T.zero] * (@size - init.size))
...
segtree = SegTree(Int32).new(data, ->(x : Int32, y : Int32) { {x, y}.max })
